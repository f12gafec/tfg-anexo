...
class RegressorWrapper(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, thresholds, base_regressor=None, **params):
        self.params = params
        # Remove the first and the last thresholds
    ...
    def predict(self, X):
        ...
        predicted_y = np.digitize(predicted_y, self.thresholds[1:-1], right=True)
    ...
    def __eq__(self, other):
        if not isinstance(other, RegressorWrapper):
            return False

        if self.base_regressor != other.base_regressor:
            return False

        if np.any(self.thresholds != other.thresholds):
            return False
